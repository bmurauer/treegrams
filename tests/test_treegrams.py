...
from dtgrams.transformers import TreeGramExtractor


class TestTreegrams(unittest.TestCase):
    def test_treegram(self):
        transformer = TreeGramExtractor("PQGram", p=2, q=3)

        documents = [
            [
                [
                    "(ROOT (INNER (INNER (DET Der) (NOUN Hund)) (VERB jagte) "
                    "(INNER (DET die) (NOUN Katze)) (. .)))"
                ]
            ]
        ]
